...
class OctoInference:
    ...
    def process_action_sequence(self, action):
        ...
        result = {
            ...
            'action_sequence': [],
            'target_poses': [],  # Changed from target_positions to target_poses
            'timestep_count': 0
        }
        ...
        if hasattr(action, 'shape'):
            ...
            if len(action.shape) == 3 and action.shape[0] == 1:  # [1, timesteps, dof]
                ...
                for t in range(timesteps):
                    ...
                    # Convert to target pose (includes position + orientation)
                    pose = self.action_to_pose(action_vector)
                    ...
                    result['target_poses'].append(pose)
                    
                    print(f"  Timestep {t+1}: x={pose['x']:.3f}, y={pose['y']:.3f}, z={pose['z']:.3f}, "
                ...
                if len(action_flat) >= 3:
                    pose = self.action_to_pose(action_flat)
                    result['target_poses'].append(pose)
    ...
    def action_to_pose(self, action_vector):
        """Convert action vector to target pose with position and orientation
        Args:
            action_vector: Single action vector [x, y, z, roll, pitch, yaw, gripper]
        Returns:
            dict with scaled target pose (position + orientation)
        """
...
def main():
    ...
    try:
        ...
        for i, pose in enumerate(result['target_poses']):
            print(f"  Pose {i+1}: x={pose['x']:.3f}, y={pose['y']:.3f}, z={pose['z']:.3f}, "
                  f"roll={pose['roll']:.3f}, pitch={pose['pitch']:.3f}, yaw={pose['yaw']:.3f}")
